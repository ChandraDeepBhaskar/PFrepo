package pfPack.util;

public class Constants {
	
	
	//if something is final then its a constant, since its value cannot be changed.. constants are written in capital letter(we can write in small letter also)..
	//whereas variable r written in small letters.
	
	
	//Application Details
	public static final String APP_URL="https://www.zoho.com/";
	public static final String BROWSER_TYPE="chrome";
	public static final String USERNAME="seleniumtraining11292017@gmail.com";
	public static final String PASSWORD="SeleniumTester$";
	
	//Path to Browser Drivers
	public static final String FIREFOX_DRIVER="drivers//geckodriver.exe";
	public static final String CHROME_DRIVER="drivers//chromedriver.exe";
	public static final String IE_DRIVER="drivers//IEDriverServer.exe";
	
	

}
